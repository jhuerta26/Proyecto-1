...
def randomGilbert(n, p, directed=False, auto=False):
    """
    Modelo aleatorio de Gilbert optimizado para grafos grandes.
    Genera aristas con probabilidad p entre pares de nodos.
    - n: número de nodos (> 0)
    - p: probabilidad (0 < p < 1)
    - directed: True si el grafo es dirigido
    - auto: True si se permiten lazos (i -> i)
    """
    p_tag = str(p).replace('.', '_')
    name = f"Gilbert_nodes_{n}_p_{p_tag}"
    graph = Graph(name, directed, auto)
    ...
    # Si se permiten lazos (i -> i)
    if auto:
        for i in range(n):
            if random() < p:
                graph.addEdge(i, i)

    # Si el grafo es no dirigido → recorrer solo mitad superior (i<j)
    if not directed:
        for i in range(n):
        ...
        # Grafo dirigido: prueba cada dirección (i != j)
        for i in range(n):
            for j in range(n):
                if i != j and random() < p:
                    graph.addEdge(i, j)
...
def randomGeografico(n, r, directed=False, auto=False):
    ...
    name = f"Geografico_nodes_{n}_distanciaMax_{int(r*10)}"
    graph = Graph(name, directed, auto)
    ...
    # Crear aristas
    for i in range(n):
        nodeSource = graph.getNode(i).attr["geoX_Y"]
        for j in range(i + 1, n):   # 👈 evita duplicados
            nodeTarget = graph.getNode(j).attr["geoX_Y"]
            d = dist(nodeSource[0], nodeTarget[0], nodeSource[1], nodeTarget[1])
            if d <= r:
                graph.addEdge(i, j)
                if directed:        # si es dirigido, puedes añadir la inversa
                    graph.addEdge(j, i)
